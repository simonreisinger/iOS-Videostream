...
Abstract: Converts depth values to grayscale values.
*/
...
import Metal
/**
 - Author: Michael Pointner, [Apple](https://developer.apple.com/library/content/samplecode/AVCamPhotoFilter/)
 - Note:
 */
class DepthToGrayscaleConverter: FilterRenderer {
    ...
    private var maxDisparity: Float!
    
    private var minDisparity: Float!
    
    private var interleavedDepthCoding: Bool!
    
    private var depthEncoding: Bool = true
    
	struct DepthRenderParam {
		var offset: Float
		var range: Float
        var depthEncoding: Int // 0 = No encoding = Grayimage, 1 = Use RGB encoding, 2 = Use interleaved RGB encoding
	}

    var range: DepthRenderParam = DepthRenderParam(offset: -4.0, range: 8.0, depthEncoding: 2)
    ...
    /**
     ...
     - Parameter outputRetainedBufferCountHint:
     */
    ...
    /**
     ...
     - Note: [AVCamPhotoFilter](https://developer.apple.com/library/content/samplecode/AVCamPhotoFilter/)
     */
	func reset() {
        ...
        if let dict = myDict {
            if (maxDisparity == nil){
                maxDisparity = (dict.value(forKey: "maxDisparity")! as! NSNumber).floatValue
            }
            if (minDisparity == nil){
                minDisparity = (dict.value(forKey: "minDisparity")! as! NSNumber).floatValue
            }
            if (interleavedDepthCoding == nil){
                interleavedDepthCoding = dict.value(forKey: "interleavedDepthCoding") as! Bool
            }
        }
	}
    ...
    /**
     Renders the Depth CVPixelBuffer into a greyscale image
     - Author: Michael Pointner, [Apple](https://developer.apple.com/library/content/samplecode/AVCamPhotoFilter/Listings/README_md.html)
     - Parameter pixelBuffer: Depth data pixel buffer
     - Returns: CVPixelBuffer with greyscale image from depthdata
     */
    func render(pixelBuffer: CVPixelBuffer) -> CVPixelBuffer? {
		...
		var max: Float = 0.0
        /*
         // To calculate min and max dynamically each frame
        ...
        var depthEncodingInt : Int = self.depthEncoding == true ? (interleavedDepthCoding! == true ? 2 : 1) : 0
        
        depthEncodingInt = 0 // TODO Remove if encoding fixed
        
        range = DepthRenderParam(offset: lowest, range: highest - lowest, depthEncoding: depthEncodingInt)
    ...
    func setDepthEncoding(value: Bool) {
        self.depthEncoding = value
    }
}
